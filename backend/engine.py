...
async def seed(db):
    ...
    for i, (name, avatar, personality) in enumerate(CREW):
        ...
        await db.agents.update_one({'id': agent['id'], 'is_bot': True}, {'$set': {'preference': preference, 'name': name}})
    await migrate(db)
    ...
    while True:
        try:
            async for agent in db.agents.find({'$or': [{'status': 'active'}, {'contest.status': 'active'}]}, {'_id': 0}):
                if not agent['is_bot'] and agent['status'] == 'active':
                    wallet = await db.wallets.find_one({'id': agent.get('owner'), 'revoked': False}, {'_id': 0, 'holder_amount': 1})
                    if not wallet or wallet['holder_amount'] <= 0:
                        await db.agents.update_one({'id': agent['id'], 'revision': agent['revision']}, {'$set': {
                            'status': 'paused', 'elapsed_before': agent['elapsed_before'] + time.time() - agent['started_at']}, '$inc': {'revision': 1}})
                        continue
