...
import {Pickaxe,Compass,UserRound,Coins,BookOpen,Wallet,ArrowUpRight,Users,Layers3,Route as RouteIcon,ShieldCheck,ArrowRight,RefreshCw,Gem,Trophy,SlidersHorizontal,History} from 'lucide-react';
...
function Shell(){
 ...
 <Route path="/agent/replay" element={<ReplayPage key={identity.wallet?.wallet_id||'public'} agent={agent} onCreate={openCreate} onConnect={()=>setWalletOpen(true)}/>}/>
 <Route path="/rewards" element={<Rewards onConnect={()=>setWalletOpen(true)}/>}/><Route path="/guide" element={<Guide agent={agent}/>}/><Route path="*" element={<div className="not-found"><Compass size={40}/><h1 data-testid="not-found-title">An unmapped tunnel.</h1><p data-testid="not-found-description">This path leads back to the mine.</p><Link className="gold-button" data-testid="not-found-home" to="/">Back to the mine<ArrowRight size={16}/></Link></div>}/></Routes>
